...
function swapSides(){
  const d=document.getElementById('weaponCompareDialog'),a=d?.querySelector('[data-compare-a]'),b=d?.querySelector('[data-compare-b]'),aSearch=d?.querySelector('[data-compare-search-a]'),bSearch=d?.querySelector('[data-compare-search-b]');
  if(!a||!b||!aSearch||!bSearch)return;
  const aValue=a.value,bValue=b.value,aQuery=aSearch.value,bQuery=bSearch.value;
  aSearch.value=bQuery;bSearch.value=aQuery;
  populateSide('a',bValue);populateSide('b',aValue);render();
}
function init(){ensureUi();document.addEventListener('click',e=>{if(e.target.closest('#compareWeaponsBtn')){open();return}if(e.target.closest('[data-compare-close]')){document.getElementById('weaponCompareDialog')?.close();return}if(e.target.closest('[data-compare-swap]')){swapSides();return}});document.addEventListener('change',e=>{if(e.target.matches?.('[data-compare-a],[data-compare-b]'))render();});document.addEventListener('input',e=>{if(e.target.matches?.('[data-compare-search-a]')){populateSide('a');render();}if(e.target.matches?.('[data-compare-search-b]')){populateSide('b');render();}});}
