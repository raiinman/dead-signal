...
class IntelligenceCompilerTests(unittest.TestCase):
    ...
    def test_compile_runs_extensions_builds_bundle_and_reports_activity(
        ...
        self.assertEqual(0, result["record_counts"]["consumer_index_files"])
        self.assertEqual(1, result["record_counts"]["reference_graph_edges"])
        self.assertGreaterEqual(result["record_counts"]["semantic_definitions"], 10)
        self.assertEqual(0, result["record_counts"]["snapshot_changed_tables"])
        ...
        self.assertEqual(8, result["record_counts"]["coverage_dashboard_fields"])
        ...
        self.assertIn("published/reports/dead-signal-coverage-dashboard.json", names)
