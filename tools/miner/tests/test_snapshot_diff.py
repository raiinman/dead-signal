...
class SnapshotDiffTests(unittest.TestCase):
 ...
 def test_large_values_are_representative_not_embedded(self):
  self.write(self.b,'large.json',{'data':{'1':{'payload':'a'*10000}}});self.write(self.c,'large.json',{'data':{'1':{'payload':'b'*10000}}});d=structured_diff(self.b/'large.json',self.c/'large.json');value=d['changed_records'][0]['fields'][0]['after'];self.assertTrue(value['truncated']);self.assertEqual(10002,value['serialized_bytes']);self.assertLess(len(json.dumps(d)),5000)
 def test_registry_backed_diff_and_patch_absence_policy(self):
  self.write(self.b,'client_data/bullet_pattern_data.json',{'data':{'Pat':{'bullet_num':1}}});self.write(self.c,'client_data/bullet_pattern_data.json',{'data':{'Pat':{'bullet_num':5}}});self.write(self.b,'base_only.json',{'data':{'1':{}}});run_table_registry(self.b,self.c,self.root,self.r);run_consumer_index(self.b,self.c,self.root,self.r);report=build_snapshot_diff(self.b,self.c,self.root,self.r);self.assertEqual(1,report['table_counts']['changed']);self.assertEqual(1,report['table_counts']['removed_or_patch_absent']);self.assertIn('projectile_count',report['affected_semantic_definitions']);self.assertIn('never asserted',report['policy']);self.assertIn('deep_detail',report['truncation'])
