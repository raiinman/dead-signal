...
Static client metadata exposes ``weapon_prototype_data`` and ``prototype_desc``
in the Weapon item-detail path. This analyzer tests only that bounded consumer
hypothesis: exact Weapon ``prototype_id`` -> exact prototype record -> exact
``prototype_desc`` -> English translation. It never executes game bytecode,
never falls back by name/similarity, never assigns VERIFIED, and never publishes.
"""
        ...
        for key_kind, key in (("raw", raw), ("marker-stripped", stripped)):
            ...
            if isinstance(translated, str) and translated.strip():
                ...
                if signature in seen:
                    continue
                seen.add(signature)
                matches.append({"source": source_name, "key_kind": key_kind, "key": key, "text": translated})
    unique_texts = sorted({row["text"] for row in matches})
    direct_text = raw if raw and not matches and " " in raw else ""
    if len(unique_texts) == 1:
        status, text = "prototype-desc-resolved-consistently", player_facing_effect(unique_texts[0], [])
    elif len(unique_texts) > 1:
        status, text = "prototype-desc-translation-conflict", ""
    elif direct_text:
        status, text = "prototype-desc-direct-text", player_facing_effect(direct_text, [])
    else:
        status, text = "prototype-desc-translation-unresolved", ""
    return {
        "status": status,
        "raw_handle": raw,
...
def _scan_consumer_evidence(report_path: Path) -> dict[str, Any]:
    ...
    if not report_path.is_file():
        return {
            "source": str(report_path), "report_present": False,
            "required_tokens": list(CONSUMER_TOKENS), "found_tokens": [],
            "all_required_tokens_found": False, "missing_tokens": list(CONSUMER_TOKENS),
            "matches": found,
    ...
    with report_path.open("r", encoding="utf-8", errors="replace") as source:
        for line_number, line in enumerate(source, 1):
            ...
            if not unresolved:
                break
    return {
        "source": str(report_path), "report_present": True,
        ...
        "missing_tokens": sorted(unresolved), "matches": found,
        "execution_policy": "Static report inspection only; game bytecode was not executed.",
        "match_policy": "Exact identifier boundaries; longer near-matches do not satisfy a required consumer token.",
        "interpretation": (
            "Exact static tokens establish a client-consumer lead for prototype-backed Weapon detail data; "
            "they do not alone prove which visible UI block uses prototype_desc."
        ),
    }
...
def run_weapon_description_consumer_trace(
    ...
    *,
    ...
    base_path = base / PROTOTYPE_TABLE
    current_path = current / PROTOTYPE_TABLE
    base_prototypes = _table(base_path) if base_path.is_file() else {}
    current_prototypes = _table(current_path) if current_path.is_file() else {}
    if not base_prototypes and not current_prototypes:
        raise ValueError(f"Weapon prototype table is missing from both snapshot layers: {PROTOTYPE_TABLE}")
    activity(
        f"UI Consumer Trace: targeted {PROTOTYPE_TABLE} — "
        f"base {len(base_prototypes)} records, current patch {len(current_prototypes)} records"
    )
    translations = _translation_sources(base, current)
    activity(f"UI Consumer Trace: loaded {len(translations)} English translation sources")

    consumer_evidence = _scan_consumer_evidence(reports_dir / PYC_REPORT)
    activity(
        "UI Consumer Trace: exact static consumer tokens "
    ...
    for index, weapon in enumerate(weapons, 1):
        ...
        row: dict[str, Any] = {
            ...
            "prototype_id": prototype_id, "name": weapon.get("name"), "category": weapon.get("category"),
        ...
        if not prototype_id or prototype_id == "0":
            ...
            row["source"] = {"table": PROTOTYPE_TABLE, "record_id": prototype_id, "field": "prototype_desc"}
        else:
            if prototype_id in current_prototypes:
                prototype, layer = current_prototypes[prototype_id], "current"
            else:
                prototype, layer = base_prototypes.get(prototype_id), "base"
            row["source"] = {
                "layer": layer, "table": PROTOTYPE_TABLE, "record_id": prototype_id,
            ...
            if not isinstance(prototype, dict):
                ...
                if pointer is None or raw_value in (None, "", 0, "0"):
                    ...
                    if (
                        resolution["status"] in {"prototype-desc-resolved-consistently", "prototype-desc-direct-text"}
                        and resolution.get("text")
                        and consumer_evidence["all_required_tokens_found"]
                    ):
                        row["consumer_backed_candidate"] = True
    ...
    activity(
        f"UI Consumer Trace complete: {counts['prototype_desc_fields_found']} prototype_desc fields; "
        f"{counts['consistent_resolutions']} consistent translations; "
        f"{counts['consumer_backed_candidates']} consumer-backed candidates"
    )
    report = {
        "schema": "dead-signal-weapon-description-ui-consumer-trace",
        "schema_version": SCHEMA_VERSION, "brand": "Dead Signal", "subject": "Weapon Description",
        "hypothesis": (
            "The client Weapon item-detail path reads weapon_prototype_data and exposes prototype_desc; "
            "therefore exact prototype_id -> prototype_desc is a bounded description-source candidate."
        ),
        "record_counts": counts,
        "consumer_evidence": consumer_evidence,
        "source_table": {
            "relative_path": PROTOTYPE_TABLE,
            "base_records": len(base_prototypes),
            "current_patch_records": len(current_prototypes),
            "overlay_policy": "Exact current record overrides exact base record; absent current IDs fall back to base.",
        },
        "weapons": rows,
        "policy": {
            ...
            "layers": "Current is a patch layer and is overlaid per exact prototype ID, never treated as a complete replacement table.",
    ...
    _write_json(reports_dir / "weapon-description-ui-consumer-trace.json", report)
    return report